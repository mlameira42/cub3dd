...
static int	validate_space(char **map, int i, int j)
{
	...
	if (j < (int) ft_strlen(map[i - 1]) && map[i - 1][j] == '0')
		return ((ft_fprintf(2, "Map not closed "), ft_exit(1)), 1);
	if (j < (int) ft_strlen(map[i + 1]) && map[i - 1][j] == '0')
		return ((ft_fprintf(2, "Map not closed "), ft_exit(1)), 1);
	if (map[i][j - 1] == '0')
		return ((ft_fprintf(2, "Map not closed "), ft_exit(1)), 1);
...
void	validate_borders(char **map, int i, int j)
{
	...
	{
		if (map[i][j] != '1')
			return (ft_fprintf(2, "Map not closed "), ft_exit(1));
	}
...
int	check_f_l(int i, int j)
{
	...
	while (glob()->map[i][++j])
	{
		if (glob()->map[i][j] != '1' && glob()->map[i][j] != ' ')
			return (ft_fprintf(2, "Map opened "), ft_exit(1), 1);
	}
	return (0);
}
...
bool	validate_map(void)
{
	...
	while (++i < glob()->rows)
	{
		...
		if ((i == 0 || i == glob()->rows - 1) && !check_f_l(i, j))
			continue ;
		if (map[i][j--] != '1')
			return (ft_fprintf(2, "Map not closed "), ft_exit(1), false);
